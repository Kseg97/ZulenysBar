...
var allTracks = [],		// An array for all the files loaded in the track
	...
	orderedPlayList = []; // Playlist in order
	uidPlaylist = []; //lista de ids de quien pide la cancion.
	numberOfSongs = 0;
	numberOfLoadedSongs = 0;
...
function getPlaylistFromDb(){
	var listOfSong = document.getElementById("playlistBD").getElementsByTagName("li");
	let numberOfSong = document.getElementById("playlistBD").getElementsByTagName("li").length;

	numberOfSongs = numberOfSong; //Get the total number of songs
	//console.log("number of songs: ");
	//console.log(numberOfSongs);
	for(var counter = 0; counter <numberOfSong; counter++){
		helperdata2(listOfSong[counter].getAttribute("audiourl"), listOfSong[counter].getAttribute("userid"));
		//console.log(orderedPlayList);
		orderedPlayList.push(listOfSong[counter].getAttribute("audiourl"));
		uidPlaylist.push(listOfSong[counter].getAttribute("userid"));
...
function helperdata2(element, id){
	var url = element;
	var userid = id;
	//console.log(url);
	var getFileBlob = function (url, cb) { //Url: es un string, cb es una funcion que se envia
        var xhr = new XMLHttpRequest();
        xhr.open("GET", url);
        xhr.responseType = "blob";
        xhr.addEventListener('load', function() {
            cb(xhr.response); //se manda xhr.response a la funcion cb
						//console.log("Ya cargo ");
						//console.log(url);
						numberOfLoadedSongs = numberOfLoadedSongs + 1;
					//	console.log("number of loaded songs :" + numberOfLoadedSongs);
						if((numberOfLoadedSongs ) >= numberOfSongs){
						//	console.log("Se cargaron todas las canciones ");
							loadIntoPlaylist();
						//	console.log(DictPlaylist);
						}
		...
		//filePathOrUrl se manda como Url
		//la funcion cb es igual a function(fileObject), entonces en cb(blobToFile)
		//la respuesta de blobToFile es igual a fileObject
		var getFileObject = function( filePathOrUrl, cb) {
		       getFileBlob(filePathOrUrl, function (blob) {
		          cb(blobToFile(blob, filePathOrUrl));
		       });
		};
// first we load the data in a map
		getFileObject( url, function(fileObject){
			DictPlaylist[url] = fileObject;

			//console.log(DictPlaylist[url]);
...
getFileObject(url, function (fileObject) {
		 getID3Data(fileObject,  function (song) {
			console.log(song);
	 		allTracks.push(song);
	 		playlist.push(song);
	 		$('#list').append($(returnTrackHTML(song, playlist.length-1)));
...
function loadIntoPlaylist(){
//	console.log();
	for(var counter = 0 ; counter<=orderedPlayList.length-1; counter ++){
	//	console.log(orderedPlayList[counter]);
		//console.log(DictPlaylist[orderedPlayList[counter]]);

		getID3Data(DictPlaylist[orderedPlayList[counter]],  function (song) {
	   console.log(song);
		 allTracks.push(song);
		 playlist.push(song);
		 $('#list').append($(returnTrackHTML(song, playlist.length-1)));
	...
	allTracks.push(url)
	playlist.push(url)
	$('#list').append($(returnTrackHTML(url,playlist.length-1)));
...
//	console.log(playlist[number].title);
	eliminate_from_db_playlist();
	let cDate = new Date();
	let strDate = cDate.getFullYear()+"/"+cDate.getMonth()+"/"+cDate.getUTCDate()+" Hour:"+cDate.getHours()+":"+cDate.getMinutes();
	console.log("datos: ");
	console.log(orderedPlayList[number]);
	console.log(uidPlaylist[number]);

	var datos=  {
		"current_playing" : orderedPlayList[number],
		"year" : cDate.getFullYear(),
		"day" : cDate.getDay(),
		"hour" : cDate.getHours(),
		"minutes" : cDate.getMinutes(),
		"seconds" : cDate.getSeconds(),
		"month" : cDate.getMonth(),//0-11
		"uid" : uidPlaylist[number],
		"date" : strDate
	}
...
function eliminate_from_db_playlist(){
	console.log('Eliminando: ');
	console.log(playlist[i].title);
	var datos = {
		"song" : playlist[i].title
	}
	$.ajax({
		type: "post",
		url: "delete_song.php",
